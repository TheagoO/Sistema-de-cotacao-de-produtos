...
public class CotacaoItemController {
	...
	public void listar() {
		...
		try {
			...
			fc.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERRO", "Cotação dos item não encontrado"));
			e.printStackTrace();
		}
	}
	...
	public void editar() {
		...
		try {
			...
			if(retorno.contains("Dados em branco") || retorno.contains("Código inválido")) {
				...
				fc.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "SUCESSO", "Cotação do item alterado!"));
				listar();
			}
			this.item = new CotacaoItem();
		} catch (Exception e) {
			fc.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERRO", "Erro ao alterar cotação"));
			e.printStackTrace();
		}
	}
	...
	public void excluir() {
		...
		try {
			...
			fc.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "SUCESSO", "Cotação do item Excluido"));
